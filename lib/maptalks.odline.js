    ...
    var ODLineLayer = function (_maptalks$ParticleLay) {
        ...
        ODLineLayer.prototype.getParticles = function getParticles(t) {
            ...
            for (var i = 0, l = this._dataToDraw.length; i < l; i++) {
                ...
                //     }
                // }
                var d = this._dataToDraw[i].durs * this.options['animationDuration'];
                var r = elapsed % d;

                if (r > 0 && this._dataToDraw[i].currIndex > -1) {
                    var j = this._dataToDraw[i].currIndex;
                    while (r > 0) {
                        var dj = this._dataToDraw[i].durations[j] * this.options['animationDuration'];
                        if (r < dj) break;
                        j++;
                    }
                    points = this._dataToDraw[i].points[j];
                    durs = this._dataToDraw[i].durations[j] * this.options['animationDuration'];
                    if (j == 0) {
                        var r2 = r % durs / durs;
                    } else {
                        var tdur = this._dataToDraw[i].durations[j - 1] * this.options['animationDuration'];
